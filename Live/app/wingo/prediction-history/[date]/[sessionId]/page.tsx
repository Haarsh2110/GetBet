...
export default function PredictionHistoryPeriodDetail() {
    ...
    useEffect(() => {
        if (!dateParam || !sessionId) return;

        const fetchDetails = async () => {
            try {
                const res = await fetch(`/api/games/wingo/history/${dateParam}/${sessionId}`, { cache: 'no-store' });
                    ...
                    <>
                        ...
                        <div className="flex flex-col items-center gap-3">
                            ...
                            <div className="bg-surface border border-white/10 px-6 py-2 rounded-full font-black text-sm tracking-widest text-[#f97316]">
                                SESSION NO. {data.sessionCountStr || '1'}
                            </div>
                        ...
                        <div className="bg-[#111111] border border-white/5 rounded-3xl p-5 shadow-2xl relative overflow-hidden shrink-0 min-h-[400px]">
                            ...
                            <div className="grid grid-cols-3 gap-2 border-b border-[#ef4444]/30 pb-3 mb-4 text-xs font-black tracking-widest text-[#ef4444] text-center">
                                <div className="text-left whitespace-nowrap">PERIOD NO.</div>
                                <div>VOLUME</div>
                                <div></div>
                            </div>

                            <div className="space-y-4">
                                {data.periods && data.periods.length > 0 ? data.periods.map((period: any) => (
                                    <div
                                        key={period.period}
                                        onClick={() => router.push(`/wingo/prediction-history/${dateParam}/${sessionId}/${period.period}`)}
                                    ...
                                    >
                                        <div className="text-left font-mono text-[13px] tracking-widest">
                                            {period.period}
                                        </div>
                                        ...
                                        <div className="flex justify-end items-center text-right pr-2">
                                            <div className="flex flex-col items-end gap-1">
                                                <span className="text-[9px] text-[#f97316]/50 uppercase font-black tracking-widest group-hover:text-[#f97316] transition-colors">
                                                    Click to<br />open
                                                </span>
                                            </div>
                                            <ArrowRight size={12} className="text-[#f97316]/50 ml-1 group-hover:text-[#f97316] group-hover:translate-x-1 transition-all" />
                                        </div>
                                    </div>
                                )) : (
