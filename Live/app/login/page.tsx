...
export default function AuthPage() {
    ...
    const handleSendOtp = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                showToast('OTP sent successfully. Enter the OTP code below.', 'info');
                setStep(2); 
            } else {
    ...
    const handleSubmit = async () => {
        if (mode === 'register') {
            ...
            if (!agreed) {
                showToast('Please agree to the privacy policy to continue.', 'info');
                return;
            }
        }
        ...
        try {
            ...
            if (data.success) {
                ...
                if (mode === 'register') {
                    showToast('Account Created. Welcome to GetBet.', 'success');
                    // Delay for toast to be seen
                    setTimeout(() => {
                        router.push('/');
                        router.refresh();
                    }, 100);
                } else {
                    showToast('Login Successful. Loading App...', 'success');
                    router.push('/');
                    router.refresh();
                }
            } else {
            ...
            showToast('Connection failed. Please check your internet or try again later.', 'error');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="flex flex-col min-h-full bg-background items-center justify-center px-4 relative overflow-hidden font-sans">
            {/* Background elements */}
            ...
            >
                <div className="w-16 h-16 bg-surface border border-primary/40 rounded-[1.5rem] flex items-center justify-center mx-auto mb-4 shadow-gold">
                    <Zap size={28} className="text-primary fill-primary/30" />
                </div>
                <h1 className="text-4xl font-black italic tracking-tighter text-white uppercase">
                    GET<span className="text-primary">BET</span>
                </h1>
                <p className="text-slate-400 text-[9px] font-black uppercase tracking-[0.4em] mt-2">Premium Gaming Platform</p>
            </motion.div>

            {/* Main Auth Container */}
            <div className="w-full max-w-sm z-10">
                {/* Tab Switcher */}
                <div className="bg-surface p-1.5 rounded-[1.5rem] flex mb-6 border border-white/5 shadow-xl">
                    <button 
                        onClick={() => handleModeSwitch('login')}
                        className={`flex-1 py-3 rounded-[1.2rem] font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 transition-all ${mode === 'login' ? 'bg-primary text-white shadow-lg' : 'text-white/40 hover:text-white'}`}
                    >
                    ...
                    <button 
                        ...
                        className={`flex-1 py-3 rounded-[1.2rem] font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 transition-all ${mode === 'register' ? 'bg-primary text-white shadow-lg' : 'text-white/40 hover:text-white'}`}
                    >
