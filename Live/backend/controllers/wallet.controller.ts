...
export async function getWallet(req: Request) {
    ...
    try {
        ...
        const payload = {
            ...
            vipBalance: (user as any).vipBalance || 0,
...
export async function walletTransfer(req: Request) {
    try {
        ...
        if (direction === 'to_betting') {
            ...
            // AUTO-EXPIRE PLAN IF BALANCE REACHES 0
            if ((user as any).vipBalance <= 0) {
                user.vipPlan = 'none';
            ...
            if ((user as any).vipBalance < amount) {
                return NextResponse.json({ error: 'Insufficient Elegible Value' }, { status: 400 });
            }
...
export async function buyPlan(req: NextRequest) {
    try {
        ...
        const Plan = (await import('@/backend/models/plan')).default;
        const Transaction = (await import('@/backend/models/transaction')).default;
        ...
        const dbPlan = await Plan.findOne({ planId, isActive: true });
        if (!dbPlan) {
        ...
        if ((user as any).mainBalance < dbPlan.price) {
            return NextResponse.json({ error: `Insufficient balance. You need ₹${dbPlan.price.toLocaleString('en-IN')}.` }, { status: 400 });
        }

        // Use the VIP Bonus from the database
        const updatedUser = await User.findOneAndUpdate(
            { userId },
            {
                $inc: { mainBalance: -dbPlan.price },
                $set: {
                    vipPlan: planId,
                    vipBalance: dbPlan.vipBonus || 0,
                }
            },
            { new: true }
        );
        ...
        await Transaction.create({
            ...
            txnId: `BUY-${dbPlan.name.split(' ')[0].toUpperCase()}-${Date.now()}`,
        });
        ...
        return NextResponse.json({
            ...
            vipBalance: updatedUser?.vipBalance
        });
    } catch (err: any) {
