...
export default function AppLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className={`${isMaster ? 'max-w-none' : 'max-w-md mx-auto'} min-h-screen w-full relative bg-background shadow-2xl overflow-hidden border-x border-white/5 flex flex-col`}>
            {/* Persist children container to prevent remounting sub-trees */}
            {splash}
            
            <div className={`flex-1 overflow-y-auto no-scrollbar ${isMaster || isMaintenance ? '' : 'pb-[72px]'} w-full h-full`}>
                {children}
            </div>
